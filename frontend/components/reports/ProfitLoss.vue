<template>
  <div class="d-flex flex-column ga-4">
    ...
    <!-- ── Empty ── -->
    <div v-else-if="!data || !data.summary" class="text-center text-medium-emphasis py-12">
    ...
    <template v-else>
      ...
      <v-card variant="outlined" rounded="lg">
        <v-card-title class="d-flex align-center ga-2 pb-2">
          ...
          <span class="text-subtitle-1 font-weight-bold section-heading">Profit and Loss Statement</span>
        </v-card-title>
        <v-divider />
          ...
          <tbody>
            <template v-for="(line, i) in data.statement" :key="i">
              ...
              <tr v-else-if="line.section === 'result'" class="result-row">
                <td class="font-weight-bold text-subtitle-2 py-3" :style="resultStyle(line.amount)">
                  {{ line.label }}
                </td>
                <td class="text-right font-weight-bold text-subtitle-1 py-3" :style="resultStyle(line.amount)">
                  {{ fmtMoney(line.amount) }}
                </td>
              </tr>
            </template>
...
// ── Computed helpers (avoid >= in template bindings) ──
...
function resultStyle(amount: number) {
...
const costBarOption = computed(() => {
  ...
  return {
    ...
    series: [{
      type: 'bar', data: items.map((c: any) => c.value), barMaxWidth: 24,
...
function exportCsv() {
  ...
  a.href = url
  a.download = `fleet-pnl-${props.data.period.start}-to-${props.data.period.end}.csv`
  a.click()
...
.v-theme--dark .section-header td { background: rgba(255,255,255,0.04) !important; }
</style>
