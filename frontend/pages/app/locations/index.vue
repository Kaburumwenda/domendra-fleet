  ...
  <div class="d-flex flex-column ga-4">
    <div class="d-flex align-center justify-space-between">
      <div class="d-flex align-center ga-3">
        ...
        <v-select v-model="filterType" :items="typeOptions" item-title="label" item-value="value" density="compact" variant="outlined" hide-details style="max-width: 200px" clearable placeholder="All types" />
        <v-btn v-if="geofenceCount" prepend-icon="mdi-shield-link-variant-outline" variant="outlined" color="primary" size="small" @click="showGeofences">Geofences ({{ geofenceCount }})</v-btn>
      </div>
      <v-btn v-can="'locations:create'" color="primary" prepend-icon="mdi-plus" size="small" @click="openCreate">Add Location</v-btn>
    </div>

    <v-card elevation="0" border rounded="lg">
      <v-data-table :headers="headers" :items="items" :loading="pending" :search="search" hover :items-per-page="20" :items-per-page-options="[10, 20, 50]">
        <template #top>
          <div class="pa-4">
        ...
        <template #item.type="{ value }"><v-chip size="small" variant="tonal">{{ typeLabel(value) }}</v-chip></template>
        <template #item.is_geofence="{ value }"><v-chip v-if="value" size="small" color="primary" variant="tonal">Geofence</v-chip></template>
        ...
        <template #item.coordinates="{ item }">
          <span class="text-body-2 text-medium-emphasis">{{ item.latitude != null ? `${item.latitude.toFixed(4)}, ${item.longitude.toFixed(4)}` : '—' }}</span>
        </template>
        <template #item.actions="{ item }">
          <div class="d-flex ga-1">
            <v-btn icon="mdi-eye-outline" variant="text" size="small" @click="openView(item)" />
            <v-btn v-can="'locations:update'" icon="mdi-pencil-outline" variant="text" size="small" @click="openEdit(item)" />
            <v-btn v-can="'locations:delete'" icon="mdi-trash-can-outline" variant="text" color="error" size="small" @click="deleteItem(item)" />
          </div>
        </template>
        <template #no-data>
          <div class="text-center py-12 text-medium-emphasis">
            <v-icon size="48" class="mb-3">mdi-map-marker-multiple</v-icon>
    ...
    <LocationLocationMap :locations="items" />
      ...
      <v-card rounded="xl">
        ...
        <v-card-text>
          <v-row dense>
            ...
            <!-- Address input with Google Places / Map / Live options -->
            <v-col cols="12">
            ...
            <v-col cols="12">
              <v-checkbox v-model="form.is_geofence" label="Is Geofence" density="compact" hide-details />
            </v-col>
            <template v-if="form.is_geofence">
              <v-col cols="6"><v-select v-model="form.shape" :items="['circle', 'polygon']" label="Shape" density="comfortable" /></v-col>
              <v-col v-if="form.shape === 'circle'" cols="6"><v-text-field v-model.number="form.radius_meters" label="Radius (m)" type="number" density="comfortable" /></v-col>
              <v-col v-if="form.shape === 'polygon'" cols="12"><v-textarea v-model="polygonText" label="Polygon points (lat,lng per line)" rows="3" density="comfortable" /></v-col>
            </template>
      ...
      <v-card rounded="xl">
        ...
        <v-card-text>
          <div class="d-flex flex-column ga-3">
            ...
            <div v-if="viewItem?.is_geofence" class="d-flex ga-4">
              <div>
                <div class="text-caption text-medium-emphasis mb-1">Geofence</div>
                <v-chip size="small" color="primary" variant="tonal">{{ viewItem?.shape || 'circle' }}</v-chip>
              </div>
              <div v-if="viewItem?.shape === 'circle'">
                <div class="text-caption text-medium-emphasis mb-1">Radius</div>
                <div class="text-body-2">{{ viewItem?.radius_meters ? viewItem.radius_meters + ' m' : '—' }}</div>
              </div>
            </div>
        ...
        <v-card-actions class="px-4 pb-4">
          ...
          <v-btn color="primary" variant="tonal" prepend-icon="mdi-pencil-outline" @click="openEditFromView">Edit</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
// ---- Address mode state (Google Places / Map / Live) ----
...
async function openMapPicker() {
  ...
  if (form.address) pickedAddress.value = form.address
...
function openView(l: any) {
  ...
  if (hasCoords(l)) nextTick(() => initViewMap(l))
}

async function initViewMap(l: any) {
  const maps = ensureMaps()
  try { await maps.ensureGoogle() } catch (e: any) { console.error('[view-map] Google Maps failed to load:', e); return }
  await nextTick()
  if (!viewMapContainer.value) return
  const center = { lat: l.latitude, lng: l.longitude }
  if (viewMapInstance) { try { viewMapInstance.setCenter(center) } catch {} }
  else { viewMapInstance = maps.createMap(viewMapContainer.value, center, 14) }
  const google = (window as any).google
  if (viewMapMarker) { try { viewMapMarker.setMap(null) } catch {} }
  viewMapMarker = new google.maps.Marker({ position: center, map: viewMapInstance, title: l.name })
  if (l.color) {
    try {
      viewMapMarker.setIcon({
        path: google.maps.SymbolPath.CIRCLE,
        scale: 8, fillColor: l.color, fillOpacity: 1,
...
const headers = [
  ...
  { title: 'Geofence', key: 'is_geofence', width: '110px' },
  { title: 'Address', key: 'address' },
  { title: 'Coordinates', key: 'coordinates', width: '180px' },
  { title: 'Status', key: 'is_active', sortable: true, width: '110px' },
  { title: 'Vehicles', key: 'vehicle_count', sortable: false, width: '100px' },
...
function defaultForm() {
  return {
    ...
    is_geofence: false, shape: 'circle', radius_meters: null,
...
// ---- Data ----
const { data: itemData, pending, refresh } = useAsyncData('locations', () =>
  $api('/locations/'), { default: () => ({ results: [], count: 0 }) }
)
const items = computed(() => {
  const all = itemData.value?.results || []
  return filterType.value ? all.filter((i: any) => i.type === filterType.value) : all
})
const geofenceCount = computed(() => items.value.filter((i: any) => i.is_geofence).length)
...
function openEdit(l: any) {
  ...
  Object.assign(form, {
    ...
    is_geofence: l.is_geofence || false, shape: l.shape || 'circle',
    radius_meters: l.radius_meters, color: l.color || '#6366f1',
    is_active: l.is_active !== false, notes: l.notes || '',
  })
  polygonText.value = (l.polygon || []).map((p: any) => Array.isArray(p) ? p.join(',') : p).join('\n')
  errors.name = ''
...
async function save() {
  ...
  try {
    ...
    const typeValue = typeof rawType === 'string' ? rawType : rawType?.value || rawType?.label || 'other'
    const body: any = {
      name: form.name, type: typeValue, address: form.address,
      latitude: form.latitude, longitude: form.longitude,
      is_geofence: form.is_geofence, shape: form.shape,
      radius_meters: form.radius_meters,
      color: form.color, is_active: form.is_active, notes: form.notes,
    }
    if (form.is_geofence && form.shape === 'polygon' && polygonText.value) {
      body.polygon = polygonText.value.split('\n').filter((l: string) => l.trim()).map((l: string) => l.split(',').map((n: string) => parseFloat(n.trim())))
    }
    if (editing.value) {
...
async function deleteItem(l: any) {
...
async function showGeofences() {
  try {
    const data = await $api('/locations/geofences/')
    itemData.value = { results: data, count: data.length }
  } catch (e) { console.error(e) }
}
</script>
